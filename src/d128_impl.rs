...
const SIGN_MASK: u128 = 0b1000_0000 << (u128::BITS - 8);
...
const SPECIAL_ENC_MASK: u128 = 0b0110_0000 << (u128::BITS - 8);
const INFINITY_MASK: u128 = 0b0111_1000 << (u128::BITS - 8);
const NAN_MASK: u128 = 0b0111_1100 << (u128::BITS - 8);
const SIGNALING_NAN_MASK: u128 = 0b0000_0010 << (u128::BITS - 8);
/// Add 2 -- two bits in front of the exponent bits are also part of the exponent, as long as they
/// are not `11`.
const EXPONENT_MASK: u128 = (1 << (u128::EXPONENT_BITS + 2)) - 1;
const EXPONENT_MAX: isize = (0b11 << u128::EXPONENT_BITS) - 1;
...
const LONG_COEFF_SHIFT: usize = u128::COEFFICIENT_BITS + 3;
const LONG_COEFF_MASK: u128 = (1 << LONG_COEFF_SHIFT) - 1;
...
const SHORT_COEFF_SHIFT: usize = u128::COEFFICIENT_BITS + 1;
const SHORT_COEFF_MASK: u128 = (1 << SHORT_COEFF_SHIFT) - 1;
const SHORT_COEFF_HIGH_BIT: u128 = 1 << LONG_COEFF_SHIFT;

impl<Ctx: crate::Context> Clone for Decimal<u128, Ctx> {
    fn clone(&self) -> Self {
        Decimal(self.0, PhantomData)
    }
}

impl<Ctx: crate::Context> Copy for Decimal<u128, Ctx> {}

impl<Ctx: crate::Context> fmt::Debug for Decimal<u128, Ctx> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "[0x{1:00$x}]", (u128::BITS as usize) / 4, self.0)
    }
}
...
impl Unpacked<u128> {
    pub(crate) fn pack_with_rounding<Ctx: crate::Context>(mut self) -> Decimal<u128, Ctx> {
        ...
        while self.coefficient >= u128::MAXIMUM_COEFFICIENT {
    ...
    fn pack<Ctx: crate::Context>(self: Self) -> Decimal<u128, Ctx> {
        ...
        if (self.coefficient & SHORT_COEFF_HIGH_BIT) == SHORT_COEFF_HIGH_BIT {
            ...
            value |= u128::from(self.exponent) << SHORT_COEFF_SHIFT;
        } else {
            value |= self.coefficient & LONG_COEFF_MASK;
            value |= u128::from(self.exponent) << LONG_COEFF_SHIFT;
        }
        Decimal(value, PhantomData)
    }
}

impl<Ctx: crate::Context> From<Decimal<u128, Ctx>> for Unpacked<u128> {
    fn from(value: Decimal<u128, Ctx>) -> Self {
        value.unpack()
    }
}

impl<Ctx: crate::Context> Decimal<u128, Ctx> {
    ...
    fn is_normal_internal(unpacked: Unpacked<u128>) -> bool {
        if unpacked.exponent >= (u128::COEFFICIENT_SIZE - 1) as u16 {
            true // Normal
        } else {
            // Check if coefficient is high enough for an exponent
            let coeff = unpacked
                .coefficient
                .checked_mul(u128::FACTORS[unpacked.exponent as usize]);
            // If overflowed, then it's guaranteed to be a "normal" number
            coeff.map_or(true, |v| v >= (u128::MAXIMUM_COEFFICIENT / 10))
        }
    }

    pub(crate) fn unpack(self) -> Unpacked<u128> {
        ...
        if unpacked.coefficient >= u128::MAXIMUM_COEFFICIENT {
    ...
    pub(crate) fn normalize_nan(self) -> Self {
        ...
        const PAYLOAD_MASK: u128 = (1 << u128::COEFFICIENT_BITS) - 1;
        ...
        return if coefficient < u128::MAXIMUM_COEFFICIENT / 10 {
    ...
    pub fn parse_rounding(s: &str, rounding: Rounding) -> Result<Self, ParseDecimalError> {
        ...
        if bytes.is_empty() {
            let exp = u128::BIAS - exponent.min(u128::BIAS);
            let value = ((exp as u128) << (u128::COEFFICIENT_BITS + 3)) | sign;
        ...
        while !bytes.is_empty() && (bytes[0] == b'.' || bytes[0].is_ascii_digit()) {
            ...
            if coeff_digits <= u128::COEFFICIENT_SIZE {
                coefficient *= 10;
                coefficient += u128::from(digit);
            } else if coeff_digits == u128::COEFFICIENT_SIZE + 1 {
                ...
                if coefficient == u128::MAXIMUM_COEFFICIENT {
                    // Essentially, divide coefficient by 10 -- it's too big now!
                    coefficient = u128::FACTORS[usize::from(u128::COEFFICIENT_SIZE) - 1];
                    exponent -= 1;
                }
                exponent -= 1;
            } else {
        ...
        if exponent < 0 {
            ...
            let can_accomodate_digits = u128::COEFFICIENT_SIZE - n_digits(coefficient);
            if coefficient == 0 {
                ...
                coefficient *= u128::FACTORS[delta];
                exponent = EXPONENT_MAX;
            }
        }
...
fn is_odd(value: u128) -> bool {
    value % 2 == 1
}

fn scale_coefficient(is_negative: bool, coefficient: &mut u128, factor: usize, rounding: Rounding) {
    let remainder = *coefficient % u128::FACTORS[factor];
    let half = u128::FACTORS[factor] / 2;
    *coefficient /= u128::FACTORS[factor];
    let round_up = match rounding {
...
impl<Ctx: crate::Context> FromStr for Decimal<u128, Ctx> {
...
fn n_digits(coefficient: u128) -> u16 {
    let bits = u128::BITS - u128::leading_zeros(coefficient);
    let digits = u128::DIGITS[bits as usize];
    if digits >= 0 {
        return digits as u16;
    }
    let digits = (-digits) as u16;
    return if coefficient < u128::FACTORS[usize::from(digits)] {
        digits
    } else {
        digits + 1
    };
...
fn increase_precision(value: &mut Unpacked<u128>, exponent: u16) {
    ...
    value.coefficient *= u128::FACTORS[factor as usize];
    value.exponent -= factor;
}

fn infinity<Ctx: crate::Context>(negative: bool) -> Decimal<u128, Ctx> {
    if negative {
        Decimal::<u128, Ctx>::NEG_INFINITY
    } else {
        Decimal::<u128, Ctx>::INFINITY
    }
}

impl<Ctx: crate::Context> Add for Decimal<u128, Ctx> {
    type Output = Self;
