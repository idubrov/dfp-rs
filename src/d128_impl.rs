...
impl<Ctx: crate::Context> From<Unpacked<u128>> for Decimal<u128, Ctx> {
    fn from(unpacked: Unpacked<u128>) -> Self {
        debug_assert!(
            unpacked.coefficient < u128::MAXIMUM_COEFFICIENT,
            "coefficient is too large"
        );
        debug_assert!(
            unpacked.exponent < (0b11 << u128::EXPONENT_BITS),
            "exponent is too large"
        );

        let mut value: u128 = if unpacked.sign { SIGN_MASK } else { 0 };
        if (unpacked.coefficient & SHORT_COEFF_HIGH_BIT) == SHORT_COEFF_HIGH_BIT {
            ...
            value |= u128::from(unpacked.exponent) << SHORT_COEFF_SHIFT;
        } else {
            value |= unpacked.coefficient & LONG_COEFF_MASK;
            value |= u128::from(unpacked.exponent) << LONG_COEFF_SHIFT;
        }
...
impl<Ctx: crate::Context> Decimal<u128, Ctx> {
    ...
    fn is_normal_internal(unpacked: Unpacked<u128>) -> bool {
        if unpacked.exponent >= (u128::COEFFICIENT_SIZE - 1) as u16 {
            ...
            let coeff = unpacked
                .coefficient
                .checked_mul(crate::consts::factors::u128[unpacked.exponent as usize]);
    ...
    pub fn parse_rounding(s: &str, rounding: Rounding) -> Result<Self, ParseDecimalError> {
        ...
        while !bytes.is_empty() && (bytes[0] == b'.' || bytes[0].is_ascii_digit()) {
            ...
            if coeff_digits <= u128::COEFFICIENT_SIZE {
                ...
                if coefficient == u128::MAXIMUM_COEFFICIENT {
                    ...
                    coefficient = crate::consts::factors::u128[u128::COEFFICIENT_SIZE - 1];
                    exponent -= 1;
                }
                exponent -= 1;
            } else {
        ...
        if exponent < 0 {
            ...
            if delta < crate::consts::factors::u128.len() {
...
fn scale_coefficient(is_negative: bool, coefficient: &mut u128, factor: usize, rounding: Rounding) {
    let remainder = *coefficient % factors::u128[factor];
    let half = factors::u128[factor] / 2;
    *coefficient /= factors::u128[factor];
    let round_up = match rounding {
